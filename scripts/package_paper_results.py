...
for rel in ['results/lira_paper','results/lira_paper_source_faithful','results/lira_pfr_tuning','results/lira_baseline_tuning','data/processed/lira']:
    src=ROOT/rel
    if src.exists():
        dst=stage/rel; dst.parent.mkdir(parents=True,exist_ok=True); shutil.copytree(src,dst)
for rel in ['configs/default.yaml','README.md','RESEARCH_PROTOCOL.md','LITERATURE_BASELINES.md','SAFEGRIP_PFR_METHOD.md','SAFEGRIP_FRC_METHOD.md','SAFEGRIP_PNTR_METHOD.md']:
    src=ROOT/rel
    if src.exists():
        dst=stage/rel; dst.parent.mkdir(parents=True,exist_ok=True); shutil.copy2(src,dst)
out=ROOT/'results/safegrip_pfr_paper_release'
